function TourDetail({ tour }) {
    ...
    const tourDetails = {
      description: 'Tour du lịch Đà Lạt 3 ngày 2 đêm sẽ đưa bạn đến với thành phố ngàn hoa lãng mạn. Khám phá những địa điểm nổi tiếng như Hồ Xuân Hương, Đồi Robin, Thung lũng Tình yêu và Thiền viện Trúc Lâm. Thưởng thức không khí trong lành và khám phá văn hóa đặc sắc của cao nguyên.',
      includes: ['Xe đưa đón', 'Khách sạn 3 sao', 'Bữa ăn theo chương trình', 'Hướng dẫn viên', 'Vé tham quan'],
      excludes: ['Chi phí cá nhân', 'Đồ uống', 'Các dịch vụ không đề cập'],
      schedule: [
        {
          day: 'Ngày 1: TP.HCM - Đà Lạt',
          activities: 'Sáng: Khởi hành từ TP.HCM đi Đà Lạt.\nTrưa: Dùng bữa trưa tại nhà hàng.\nChiều: Tham quan Hồ Xuân Hương, Quảng trường Lâm Viên.\nTối: Khám phá chợ đêm Đà Lạt, thưởng thức ẩm thực địa phương.'
        },
        {
          day: 'Ngày 2: Khám phá Đà Lạt',
          activities: 'Sáng: Tham quan Đồi Robin, Thiền viện Trúc Lâm.\nTrưa: Dùng bữa tại nhà hàng.\nChiều: Tham quan Thung lũng Tình yêu, Vườn hoa thành phố.\nTối: Tự do khám phá.'
        },
        {
          day: 'Ngày 3: Đà Lạt - TP.HCM',
          activities: 'Sáng: Tham quan Làng Cù Lần, Thác Datanla.\nTrưa: Dùng bữa trưa.\nChiều: Trở về TP.HCM, kết thúc tour.'
        }
      ],
      reviews: [
        { id: 1, user: 'Nguyễn Văn A', rating: 5, comment: 'Tour rất tuyệt vời, hướng dẫn viên nhiệt tình', date: '15/03/2025' },
        { id: 2, user: 'Trần Thị B', rating: 4, comment: 'Dịch vụ tốt, lịch trình hợp lý nhưng hơi mệt', date: '10/03/2025' },
        { id: 3, user: 'Lê Văn C', rating: 5, comment: 'Đồ ăn ngon, khách sạn sạch sẽ', date: '05/03/2025' },
      ],
      bookings: [
        { id: 1, customer: 'Nguyễn Văn Nam', phone: '0912345678', participants: 2, startDate: '15/05/2025', status: 'confirmed' },
        { id: 2, customer: 'Trần Thị Hương', phone: '0987654321', participants: 4, startDate: '20/05/2025', status: 'confirmed' },
        { id: 3, customer: 'Lê Minh Dương', phone: '0977123456', participants: 1, startDate: '25/05/2025', status: 'pending' },
      ]
    };
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden p-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <img src="/api/placeholder/800/400" alt={tour.name} className="w-full h-64 object-cover rounded-lg" />
              <div className="mt-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-medium ${tour.status === 'active' ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="space-y-4">
              {tourDetails.schedule.map((day, index) => (
                <div key={index} className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <p className="text-gray-600 whitespace-pre-line">{day.activities}</p>
                </div>
              ))}
            </div>
          </div>
                  ...
                  {tourDetails.bookings.map((booking) => (
                    <tr key={booking.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        >
                          {booking.status === 'confirmed' ? 'Xác nhận' : 'Chờ duyệt'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button className="text-blue-600 hover:text-blue-900 mr-2">Xem</button>
                        <button className={`${booking.status === 'pending' ? 'text-green-600 hover:text-green-900' : 'text-gray-400 cursor-not-allowed'} mr-2`}>
                          {booking.status === 'pending' ? 'Xác nhận' : 'Đã xác nhận'}
                        </button>
